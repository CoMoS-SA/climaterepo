##############################################
# This script starts from the hourly gridded files of precipitation
# to produce the daily gridded file by summing up the values (in case of the precipitation)
##############################################
...
os.chdir(r"/users/testa/Documents/Climate/hourly.prec") 
...
for y in years:
    for m in months:
        filesAPI.append("download_hourly_total_precipitation_" + str(y) + "_" + m + ".nc") 
...
for file in filesAPI:
    ...
    for group_id in range(num_groups):
        ...
        group_stat = ds.isel(time=slice(start_idx, end_idx)).sum(dim='time') 
        group_stat = group_stat*1000 
    ...
    os.chdir(r"/users/testa/Documents/Climate/daily.prec")
